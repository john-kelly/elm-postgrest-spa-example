...
fetch token page feedSource =
    let
        ...
        listConfig =
            { defaultListConfig | page = page, size = articlesPerPage }
            ...
            case feedSource of
                YourFeed username ->
                    let
                        ...
                        feedConfig =
                            { defaultFeedConfig | page = page, size = articlesPerPage }
                    in
                    token
                        |> Maybe.map (Request.Article.feed feedConfig username >> Http.toTask)
    ...
    in
    task
        |> Task.map (\pageData -> ( page, Feed pageData.data pageData.count  ))
